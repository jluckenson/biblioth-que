<?php
    ...
    function add($titre,$auteur,$stock,$cat,$p_ach,$desc,$image){
		global $bdd;
		$req = $bdd->prepare('INSERT INTO livres(titre,auteur,p_ach,dispo,stock,cat,nb_v, disc ,promo,image) VALUES(:titre,:auteur,:p_ach,"oui",:stock,:cat ,"0", :desc ,0, :image)');
		$req->execute(array(
			'titre'=>$titre,
			'auteur'=>$auteur,
			'stock'=>$stock,
    ...
    function supp($id){
		...
		$bdd->query ('ALTER TABLE livres AUTO_INCREMENT=0');
	}
    //modifier le livre
    function modifier($id,$titre,$auteur,$dispo,$stock,$cat,$p_ach,$disc,$image){
		global $bdd;
		$donnees=recherche_livre($id);
		$rep=$bdd->prepare('UPDATE livres SET titre=:titre,auteur=:auteur,p_ach=:p_ach,dispo=:dispo,stock=:stock,cat=:cat,nb_v=:nb_v,disc=:disc,image=:image where id = '.$id);
		$rep->execute(array(
			'titre'=>$titre,
			'auteur'=>$auteur,
			'dispo'=>$dispo,
